...
# WORKDIR /src
# COPY API_Identity.csproj API_Identity/
# RUN dotnet restore API_Identity/API_Identity.csproj
# COPY . .
# WORKDIR /src/API_Identity
# RUN dotnet build API_Identity.csproj -c Release -o /app

# FROM build AS publish
# RUN dotnet publish API_Identity.csproj -c Release -o /app

# FROM base AS final
# WORKDIR /app
# COPY --from=publish /app .
# ENTRYPOINT ["dotnet", "API_Identity.dll"]
...
# Build the image:
# docker build -f apiIdentity.production.dockerfile -t [yourDockerHubID]/dotnet:1.0.0
# docker push
